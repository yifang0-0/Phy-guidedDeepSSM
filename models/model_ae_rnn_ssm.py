...
class AE_RNN(nn.Module):
    def __init__(self, param, device,  sys_param={},  dataset="toy_lgssm", bias=False, ):
        ...
        self.rnn = nn.GRU(self.h_dim, self.h_dim, self.n_layers, bias,bidirectional = True)
    ...
    def forward(self, u, y, u_norm_dict, y_norm_dict):
        ...
        for t in range(seq_len):
            ...
            if self.mpnt_wt>100:
                ...
                # print("x_mean_nn.shape", x_mean_nn.shape)
                # print("xtm1.shape", x_tm1.shape)
    ...
    def generate(self, u, u_norm_dict, y_norm_dict):
        ...
        for t in range(seq_len):
            # torch.autograd.set_detect_anomaly(True)
            if t == 0:
                x_tm1 =  torch.zeros(batch_size, self.z_dim, device=self.device)+5
                # x_tm1[:, 1] = 4.9728
                # x_tm1[:,0] = 6.4158
