...
class MLP(nn.Module):
    ...
    def forward(self, u, y, u_norm_dict, y_norm_dict):
        ...
        x = torch.ones(batch_size, self.z_dim, seq_len, dtype=torch.float32, device=self.device)
        ...
        for t in range(seq_len):
            ...
            if t == 0:
                x_tm1 =  x[:,:,t].clone()
                # x_tm1 =  x[:,:,t].clone() + self.param["x0"].to(self.device)
            ...
            if self.mpnt_wt>100:
                ...
                #physics augmented
                dynn_phi = self.dynn(torch.cat([phi_u_t, phi_xm1_t], 1))
        ...
        for t in range(seq_len):
            ...
            if self.mpnt_wt>100:
                ...
                if x_mean_phy.dtype != self.x_phi_phy[0].weight.dtype:
                    ...
                    x_phy_phi = self.x_phi_phy(x_mean_phy)
                dynn_phi = self.dynn_phy(torch.cat([phi_u_t, phi_xm1_t,x_phy_phi], 1))
                x_mean_nn = self.x_mean(dynn_phi)
                x_t = x_mean_nn
