...
def analyze_training_timing():
    ...
    for config, log_file in log_files.items():
        ...
        # Estimate timing
        estimate_timing(training_info, config)
...
def estimate_timing(training_info, config):
    ...
    return {
        ...
        'total_remaining_time': total_remaining_time if training_info['num_rounds'] < 10 else 0,
        'completion_time': completion_time if training_info['num_rounds'] < 10 else None
    }

def save_timing_analysis(results, output_file="training_timing_analysis.csv"):
    """
    Save timing analysis results to CSV
    """
    rows = []
    ...
    for config, info in results.items():
        row = {
            ...
            'Avg_Epochs_Per_Round': info['avg_epochs_per_round'],
            'Max_Epochs': info['max_epochs'],
            'Min_Epochs': info['min_epochs'],
            'Estimated_Total_Time_Seconds': info.get('total_estimated_time', 0),
            'Estimated_Remaining_Time_Seconds': info.get('total_remaining_time', 0),
            'Estimated_Completion_Time': info.get('completion_time', 'N/A')
        }
        rows.append(row)
...
def print_detailed_round_info(results):
    """
    Print detailed information about each training round
    """
    print("\n" + "=" * 80)
    ...
    for config, info in results.items():
        ...
        for round_info in info['rounds']:
            print(f"  Round {round_info['round']}: {round_info['epochs']} epochs")
        
        print(f"  Summary: {info['num_rounds']} rounds, {info['total_epochs']} total epochs")
...
if __name__ == "__main__":
    ...
    # Print detailed round information
    print_detailed_round_info(results)
    
    # Save results to CSV
    df_timing = save_timing_analysis(results)
